...
- [ ] **P3 — a single degenerate embedding vector fails its whole batch, permanently.**
  **What:** the #4616 gateway guard throws `AIConfigError` for a zero-norm / non-finite vector and `embed()` has no per-sub-batch isolation, so one degenerate input (deterministic for some local models on whitespace- or emoji-only chunks) fails up to 100 unrelated chunks and is retried every cycle; the facts drain records only the batch error. **Why:** review of v0.50.2.0; the guard is correct (HNSW would silently skip the row) but the blast radius is a design call. **Fix:** on backfill paths, retry a failed sub-batch one vector at a time and skip only the degenerate input with a named warning. **Effort:** M. **Priority:** P3.
