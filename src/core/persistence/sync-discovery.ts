...
export async function discoverManagedSync(engine: BrainEngine, opts: SyncOpts): Promise<SyncDiscovery> {
  ...
  const eligible = (path: string) => (!scope || path.startsWith(`${scope}/`)) &&
    ...
    isSyncable(path, { strategy: strategy as 'markdown', includeHidden: opts.includeHidden });
  ...
  const discovered: SyncDiscovery = { binding: { ...binding, owner_epoch: String(binding.owner_epoch), topology_generation: String(binding.topology_generation) }, root, gitRoot, sourceId, incarnation: source.incarnation, from: source.last_commit, target, entries: selected, slugMode };
