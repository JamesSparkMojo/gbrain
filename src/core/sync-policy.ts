 ...
 * filter (sync.ts), autopilot's freshness dispatcher (autopilot.ts) and the
 * full-cycle fan-out (autopilot-fanout.ts) all read it here so they cannot
 * drift apart. It deliberately does NOT gate performSync() itself — an
