...
/**
 * Lock id for the `embed --stale --facts` drain (#4867 review). Its own key,
 * NOT `embedBackfillLockId('facts')`: a source may legitimately be named
 * `facts`, and the facts pass must neither block nor be blocked by the chunk
 * drain. ponytail: one global key regardless of --source scope, so an
 * all-source run and a scoped run stay mutually exclusive without the chunk
 * path's sorted per-source acquire; per-source keys if concurrent scoped
 * facts drains ever matter.
 */
export function embedFactsBackfillLockId(): string {
  return 'gbrain-embed-facts-backfill';
}
