...
describe('complete CI timing extraction', () => {
  ...
  it('requires the outer capture completion after a passing Bun summary', () => {
    const captured = unitLog.replaceAll('\tstep\t', '\tcapture\t');
    ...
    expect(mineWeights(start + captured + complete, 'unit').size).toBe(2);
